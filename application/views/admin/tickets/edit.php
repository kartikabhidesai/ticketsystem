        ...
        <form method="post" class="form-horizontal"  enctype="multipart/form-data"  action="<?= admin_url(); ?>tickets/edit/<?=  $this->utility->encode($getTicket[0]->id);?>" id='ticketEditForm'>
            ...
            <div class="form-group headingmain">                        
                <h2 class="title" style="margin:10px">Ticket Details - <?= $getTicket[0]->ticket_code; ?>  
                        <a href="<?= admin_url().'tickets/view/1'; ?>" style="margin:10px" class="btn btn-sm btn-primary pull-right m-t-n-xs" ><strong><i class="fa fa-tag"></i> View Tickets </strong></a> 
                </h2> 
                ...
                <div class="col-sm-7">
                      <select class="form-control m-b" name="client_id">
                        ...
                        <?php for($i=0; $i<count($reporter_detail); $i++){ ?>
                            <option value="<?= $reporter_detail[$i]->id;?>" <?php if($reporter_detail[$i]->id == $getTicket[0]->client_id){ echo "selected='selected'";} ?>><?= $reporter_detail[$i]->first_name;?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
