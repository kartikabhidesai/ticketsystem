...
//        exit;
        if (!empty($row)) {
            ...
            $data['update']['password'] = md5($auto_pass);
            $data['where'] = ['id' => $row['id']];
            $data['table'] = TABLE_USER;
            $result = $this->updateRecords($data);
            $data['username'] = $row['first_name'] . ' ' . $row['last_name'];
            $data['email'] = $row['email'];
            $data['password'] = $auto_pass;
            $data['message'] = $this->load->view('email_template/forgot_password', $data, true);
            $data['from_title'] = 'New password';
            $data['subject'] = 'Forgot Password '.PROJECT_NAME;
            $data["to"] = $row['email'];
            $this->utility->sendMailSMTP($data);
            $json_response['status'] = 'success';
            $json_response['message'] = 'Well done Login Successfully Done';
            ...
            $json_response['status'] = 'error';
            $json_response['message'] = 'Email address does not match';
        }
        return $json_response;
    }
